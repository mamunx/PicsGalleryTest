...
abstract class PhotoDb : RoomDatabase() {
    companion object {
        fun create(context: Context, useInMemory: Boolean): PhotoDb {
            val databaseBuilder = if (useInMemory) {
                Room.inMemoryDatabaseBuilder(context, PhotoDb::class.java)
            } else {
                Room.databaseBuilder(context, PhotoDb::class.java, "pics_gallery.db")
            }
    ...
    abstract fun remoteKeys(): PhotoRemoteKeyDao
}